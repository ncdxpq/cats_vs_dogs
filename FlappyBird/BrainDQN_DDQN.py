...
OBSERVE = 1000000.  # timesteps to observe before training. too large : for test
...
class BrainDQN:
    ...
    def trainQNetwork(self):
        ...
        # Step 1: obtain random minibatc.h from replay memory
        ...
        # Step 2: calculate y
        y_batch = []
        current_Q_batch = self.QValue.eval(feed_dict={self.stateInput: nextState_batch})
        max_action_next = np.argmax(current_Q_batch, axis=1)  # 先在当前Q网络中找出最大Q值对应的动作
        target_Q_batch = self.QValueT.eval(feed_dict={self.stateInputT: nextState_batch})
        for i in range(0, BATCH_SIZE):
            terminal = minibatch[i][4]
            if terminal:
                y_batch.append(reward_batch[i])
            else:  # 利用这个选择出来的动作在目标网络中去计算Q值
